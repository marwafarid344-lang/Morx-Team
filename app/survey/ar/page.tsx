...
import { ChevronRight, ChevronLeft, Send, Loader2, Gift, ArrowRight, Sparkles, Check } from "lucide-react"
    ...
    <>
      <div className="fixed top-0 left-0 right-0 h-[3px] bg-muted/50 z-50 backdrop-blur-sm">
        <motion.div
          ...
          style={{ background: `linear-gradient(90deg,${accent},${accent2})`, willChange: "width", marginLeft: "auto" }}
        >
          <div className="absolute left-0 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full" style={{ background: accent2, boxShadow: `0 0 12px ${accent2}88, 0 0 24px ${accent2}44` }} />
        </motion.div>
      </div>
...
/* ═══════════════════════════════════════════════════════════════════════════════
   Main Survey Page — Arabic (RTL)
   ═══════════════════════════════════════════════════════════════════════════ */
export default function SurveyPageAR() {
  ...
  /* ── Translate Arabic answers to English before submission ── */
  const translateValue = (val: string): string => AR_TO_EN_MAP[val] ?? val
    ...
    try {
      ...
      for (const [qId, entryId] of Object.entries(FORM_MAP)) {
        ...
        if (Array.isArray(val)) {
          for (const v of val) {
            const sendVal = (v === "أخرى" && otherText.trim()) ? otherText.trim() : translateValue(v)
          ...
          else finalVal = translateValue(finalVal)
      ...
      toast.success("✅ تم إرسال ردك! إجاباتك اتسجلت بنجاح.")
    } catch {
      toast.error("⚠️ حصلت مشكلة في الإرسال. ممكن إجاباتك ماتسجلتش. حاول تاني.")
    }
  ...
  return (
    <div dir="rtl" lang="ar" className="relative min-h-screen w-full overflow-hidden bg-background font-cairo">
          ...
          {step === 0 && (
            <motion.div key="intro" {...FADE_UP} transition={DUR}>
              <div className="flex items-center gap-3 mb-8">
                ...
                <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-muted-foreground/40">بحث</span>
              </div>

              <h1 className="font-black italic tracking-tighter leading-[0.88] text-foreground mb-4" style={{ fontSize: "clamp(2.4rem,8vw,7rem)" }}>
                AI يقدر يكتب.<br />
                <span className="bg-clip-text text-transparent bg-gradient-to-l from-primary via-secondary to-primary bg-[length:200%_100%] animate-[shimmer_3s_ease-in-out_infinite]">
                  بس يقدر يحس؟
                </span>
              </h1>

              <p className="text-base md:text-lg text-muted-foreground/60 max-w-xl mb-4 leading-relaxed">
                اقرأ، قارن، واحكم. تقدر تفرق بين الـ AI والإنسان؟
              </p>

              <motion.div
                className="h-[2px] rounded-full bg-gradient-to-l from-primary via-secondary to-transparent mb-10"
                initial={{ width: 0 }}
                animate={{ width: "35%" }}
              ...
              <div className="mb-8 p-5 rounded-2xl bg-background/60 backdrop-blur-xl border border-border/30 flex items-center gap-4 max-w-xl shadow-lg">
                ...
                <p className="text-sm md:text-base text-foreground/70 font-medium">
                  كمّل الاستبيان عشان تكسب جوائز حصرية 🎁
                </p>
              </div>
              ...
              {q.type === "text-display" && q.textContent && (
                <>
                  ...
                  <div className="p-6 md:p-8 rounded-3xl bg-background/50 backdrop-blur-xl border border-border/30 max-w-2xl relative overflow-hidden shadow-lg"
                    style={{ borderRight: `3px solid ${q.accent}` }}>
                    <div className="absolute top-0 left-0 w-32 h-32 rounded-full blur-[60px] opacity-10" style={{ background: q.accent }} />
                      ...
                      return (
                        ...
                        >
                          <p className="text-sm md:text-base text-muted-foreground/70 font-medium mb-4">
                            <span className="inline-flex items-center justify-center size-5 rounded-full text-[10px] font-bold ml-2" style={{ background: `${q.accent}15`, color: q.accent }}>{idx + 1}</span>
                            {sq.label}
                          </p>
                    ...
                    <motion.div className="max-w-md" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                      <div className="relative">
                        ...
                        <motion.div
                          ...
                          style={{ background: `linear-gradient(270deg, ${q.accent}, ${q.accent2})` }}
                          initial={false}
                      ...
                      {answer === "أخرى" && (
                        <motion.div initial={{ opacity: 0, y: 10, filter: "blur(4px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.3 }}>
                          <div className="relative mt-5 max-w-md">
                            <input
                              type="text" placeholder="حدد مجالك…" value={otherText}
                            ...
                            />
                            <div className="h-[2px] bg-border/20 rounded-full" />
                            <motion.div className="absolute bottom-0 right-0 h-[2px] rounded-full" style={{ background: q.accent }}
                    ...
                    <div className="max-w-3xl">
                      ...
                      {Array.isArray(answer) && (answer as string[]).includes("أخرى") && (
                        <motion.div initial={{ opacity: 0, y: 10, filter: "blur(4px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.3 }}>
                          <div className="relative mt-5 max-w-md">
                            <input
                              type="text" placeholder="حدد…" value={otherText}
                              onChange={(e) => setOtherText(e.target.value)} autoFocus
                              className="w-full bg-transparent text-foreground text-lg font-light placeholder-muted-foreground/20 outline-none pb-3"
                              style={{ caretColor: q.accent }}
                            />
                      ...
                      <div className="flex items-center justify-between mt-2">
                        {q.required ? (
                          <p className="text-[11px] text-muted-foreground/30">
                            {((answer as string) || "").trim().length} حرف
                            {((answer as string) || "").trim().length < 5 && <span style={{ color: q.accent }}> · الحد الأدنى 5</span>}
                          </p>
                        ) : (
                          <p className="text-[11px] text-muted-foreground/25">اختياري</p>
                        )}
          ...
          {step === TOTAL + 1 && (
            <motion.div key="done" {...FADE_UP} transition={DUR} className="text-center flex flex-col items-center">
              ...
              >
                ...
                <Sparkles className="absolute -top-1 -left-1 size-6 text-secondary animate-pulse" />
                <Sparkles className="absolute -bottom-1 -right-2 size-4 text-primary/60 animate-pulse" style={{ animationDelay: "0.5s" }} />
              </motion.div>
              ...
              <p className="text-lg md:text-xl text-muted-foreground/60 max-w-lg mb-4 leading-relaxed">
                رأيك بيساعدنا نفهم إزاي الناس بتشوف كتابة الذكاء الاصطناعي.
              </p>
              <p className="text-sm text-muted-foreground/30 max-w-lg mb-14">
                كل الإجابات مجهولة • مقدم من Morx
              </p>
              ...
              <Link href="/"
                ...
                الرجوع لـ Morx <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </Link>
            </motion.div>
          )}
